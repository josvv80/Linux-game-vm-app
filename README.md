...
- direct Sunshine stream-host probe actions from the host UI without launching a game, including checked-at/mode detail, normalized probe target and timing input for both tests and saved scenarios, adding observed process/port targets back into the scenario config, explicit reset controls for saved target lists, resetting empty saved target lists to provider defaults, and identifying when those targets are already covered
