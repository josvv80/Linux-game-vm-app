...
- direct Sunshine stream-host probe actions from the host UI without launching a game, including normalized probe target input for both tests and saved scenarios, adding observed process/port targets back into the scenario config, and identifying when those targets are already covered
