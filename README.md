...
- direct Sunshine stream-host probe actions from the host UI without launching a game, including normalized probe target input, adding observed process/port targets back into the scenario config, and identifying when those targets are already covered
